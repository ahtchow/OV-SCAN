...
class AlignmentSelector:
    ...
    def __init__(self, selection_config, data_root, tag="train"):
        ...
        # Minimum percent of the segmentation mask that must be covered by the object
        # for the object to be sufficiently occupying the image crop.
        self.GROUNDING_SAM_FITNESS_THRESHOLDS = {
            ...
            # Additionals
            "animal": 0.5,
            "standing traffic sign": 0.25,
            "standing traffic crossing light": 0.25,
            "fire hydrant": 0.25,
            "mail box": 0.25,
            "electrical box": 0.25,
            "parking meter": 0.25,
    ...
    def get_alignment_status(self, image_crops, gs_info):
        ...
        for image_crop in image_crops:
            ...
            cam_id = image_crop['cam_id']
            gd_detections = gs_info[cam_id]
            gs_det_shapes = []
            gs_det_instance_id = []
            gs_selected_index = []
            instance_mask = np.array(Image.open(self.data_root / gd_detections['instance_mask_path']))
            if not gd_detections['detections']:
                continue

            for gs_index, gs_det in enumerate(gd_detections['detections']):
            ...
            for index, gs_det_shape in enumerate(gs_det_shapes):
                ...
                if dist < search_radius and dist < object_of_interest['matching_distance'] and box_similarity > self.BOX_SIMILARITY_THRESH:
                    ...
                    object_of_interest['object'] = gd_detections['detections'][gs_selected_index[index]]
                    ...
                    object_of_interest['cam_id'] = image_crop['cam_id']
            ...
            # # save image crops to png
            # if object_of_interest['status'] == AlignmentStatus.FIT_FOR_ALIGNMENT:
            #     Image.fromarray(object_of_interest['image_crop']).save(f"image_crops_{object_of_interest['box_index']}_mask.png")
    ...
    def compute_fitness(self, selection_result, instance_mask):
        ...
        if selection_result['fitness'] > self.GROUNDING_SAM_FITNESS_THRESHOLDS[ooi_label.replace('.','')]:
            ...
            object_crop = np.array(Image.open(self.data_root / selection_result['img_path']))[y_min:y_max, x_min:x_max]
