...
class TransFusionHeadKitti(nn.Module):
    ...
    def __init__(
        ...
        # Alignment Head and Selector
        self.alignment_head = FFN(input_dim=hidden_channel, 
                                  hidden_dim=self.model_cfg.ALIGNMENT.HIDDEN_DIM, 
                                  output_dim=self.model_cfg.ALIGNMENT.CLIP_EMBED_DIM)
        ...
        self.kitti_to_ov_classes = self.model_cfg.ALIGNMENT.KITTI_TO_OV_CLASSES
        self.ov_classes = []
        for class_name in self.kitti_to_ov_classes.values():
            self.ov_classes.extend(class_name)
        self.ov_to_kitti_classes = {class_name: key for key, value in self.kitti_to_ov_classes.items() for class_name in value}
        self.kitti_to_label_id = {class_name: idx+1 for idx, class_name in enumerate(self.class_names)}
    ...
    def predict(self, inputs):
        ...
        global_query_feat = self.decoder(
        ...
        res_layer["object_feats"] = global_query_feat.permute(0, 2, 1)

        # Predict class for each object, aligned with normalized image features
        res_layer['clip_preds'] = self.alignment_head(res_layer["object_feats"])
    ...
    def forward(self, batch_dict):
        ...
        if not self.training:
            ...
            loss, tb_dict = self.loss(gt_bboxes_3d, gt_labels_3d, res, gt_grounding_sam)
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, pred_dicts, gt_grounding_sam, **kwargs):
        ...
        one_hot_targets = torch.zeros(*list(labels_for_loss.shape), self.num_classes+1, dtype=cls_score.dtype, device=labels_for_loss.device)
        ...
        alignment_batch = zip(batch_ids, pos_indices, bbox_grounding_sam)

        gt_labels_interest = []
        pred_labels_interest = []

        for batch_id, pos_ind, grounding_sam in alignment_batch:
    ...
    def decode_bbox(self, heatmap, rot, dim, center, height, vel, 
        ...
        for i in range(heatmap.shape[0]):
            ...
            kitti_labels = [self.kitti_to_label_id[self.ov_to_kitti_classes[self.ov_classes[label]]] for label in predictions_dict['ov_labels']]
            predictions_dict["pred_labels"] = torch.tensor(kitti_labels).cuda()
    ...
    def get_bboxes(self, preds_dicts, filter=True):
        ...
        one_hot = F.one_hot(
            preds_dicts['query_labels'], num_classes=self.num_classes
        ).permute(0, 2, 1)
