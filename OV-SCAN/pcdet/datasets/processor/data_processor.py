...
class DataProcessor(object):
    ...
    def filter_alignment_objects(self, data_dict=None, config=None):
        """
        Filter Alignment Objects based on the distance between the objects and the ego vehicle
        Smaller object need sufficient resolution, while larger objects can be aligned to at further distance
        """
        if data_dict is None:
            self.min_pixels = config.MIN_PIXELS
            return partial(self.filter_alignment_objects, config=config)

        if not self.training:
            return data_dict

        gt_grounding_sam = data_dict['gt_grounding_sam']
        gt_names = data_dict['gt_names']

        for i in range(len(gt_grounding_sam)):
            if gt_grounding_sam[i] is None:
                continue

            (xmin, ymin), _, (xmax, ymax), _ = gt_grounding_sam[i]['pred_corners']
            num_pixels = (xmax - xmin) * (ymax - ymin)
            class_label = gt_names[i]

            if class_label == '-1': # should not be the case, filtered by gt_mask
                gt_grounding_sam[i] = None
                gt_names[i] = '0'
                continue
        ...
        assert len(data_dict['gt_grounding_sam']) == len(data_dict['gt_names'])
        return data_dict
