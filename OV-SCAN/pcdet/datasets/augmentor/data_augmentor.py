...
from . import augmentor_utils, database_sampler, database_ov_sampler
...
class DataAugmentor(object):
    ...
    def gt_ov_sampling(self, config=None):
        db_sampler = database_ov_sampler.DataBaseOVSampler(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger
    ...
    def forward(self, data_dict):
        ...
        if 'gt_boxes_mask' in data_dict:
            ...
            # OV mods
            if 'gt_size_labels' in data_dict:
                data_dict['gt_size_labels'] = data_dict['gt_size_labels'][gt_boxes_mask]
            if 'gt_grounding_sam' in data_dict:
                data_dict['gt_grounding_sam'] = data_dict['gt_grounding_sam'][gt_boxes_mask]
