...
class DataBaseOVSampler(object):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        gt_grounding_sam = data_dict['gt_grounding_sam'][gt_boxes_mask]
        ...
        sampled_gt_grounding_sam = np.array([x['gt_grounding_sam'] for x in total_valid_sampled_dict])
        ...
        gt_grounding_sam = np.concatenate([gt_grounding_sam, sampled_gt_grounding_sam], axis=0)
        data_dict['gt_boxes'] = gt_boxes
        data_dict['gt_names'] = gt_names
        data_dict['gt_grounding_sam'] = gt_grounding_sam
    ...
    def __call__(self, data_dict):
        ...
        """
        # Check if 'gt_boxes' exists and has data
        if 'gt_boxes' in data_dict and data_dict['gt_boxes'].size > 0:
            gt_boxes = data_dict['gt_boxes']
            gt_names = data_dict['gt_names'].astype(str)
            existed_boxes = gt_boxes.copy()
        else:
            # Initialize empty gt_boxes and gt_names
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                if existed_boxes.shape[0] != 0:
                    ...
                    # If no existing boxes, only check for overlaps among sampled boxes
                    iou2 = iou3d_nms_utils.boxes_bev_iou_cpu(sampled_boxes[:, 0:7], sampled_boxes[:, 0:7])
                    iou2[range(sampled_boxes.shape[0]), range(sampled_boxes.shape[0])] = 0
                ...
                valid_sampled_boxes = sampled_boxes[valid_indices]

                if existed_boxes.shape[0] != 0:
                    existed_boxes = np.concatenate((existed_boxes, valid_sampled_boxes[:, :existed_boxes.shape[-1]]), axis=0)
                else:
                    existed_boxes = valid_sampled_boxes[:, :7]  # Initialize existed_boxes with sampled boxes

                total_valid_sampled_dict.extend(valid_sampled_dict)

        # Determine sampled_gt_boxes based on the number of original gt_boxes
        sampled_gt_boxes = existed_boxes[gt_boxes.shape[0]:, :] if gt_boxes.shape[0] > 0 else existed_boxes
        ...
        # Remove 'gt_boxes_mask' if it exists
