...
from nuscenes.eval.agnostic_detection.data_classes import DetectionMetricData
...
def accumulate(gt_boxes: EvalBoxes,
    ...
    pred_boxes_list = [box for box in pred_boxes.all] # do not require classes to match (class agnostic)
    ...
    for ind in sortind:
        ...
        if is_match:
            ...
            # Traffic Cones no orientation. So we discard the orientation error.
            if gt_box_match.detection_name == 'traffic_cone':
                match_data['orient_err'].append(0.)
                match_data['attr_err'].append(np.nan)
                match_data['vel_err'].append(0.)

            elif gt_box_match.detection_name == 'barrier':
                match_data['attr_err'].append(np.nan)
                match_data['vel_err'].append(0.)
