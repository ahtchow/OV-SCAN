...
class BBox:
    ...
    def __str__(self):
        return 'x: {}, y: {}, z: {}, heading: {}, length: {}, width: {}, height: {}, vx: {}, vy: {}, score: {}'.format(
            self.x, self.y, self.z, self.o, self.l, self.w, self.h, self.vx, self.vy, self.s)
    ...
    def bbox2dict(cls, bbox):
        return {
            ...
            'vx': bbox.vx, 'vy': bbox.vy, 'score': bbox.s}
    
    @classmethod
    def bbox2array(cls, bbox):
        if bbox.s is None:
            return np.array([bbox.x, bbox.y, bbox.z, bbox.o, bbox.l, bbox.w, bbox.h, bbox.vx, bbox.vy])
        else:
            return np.array([bbox.x, bbox.y, bbox.z, bbox.o, bbox.l, bbox.w, bbox.h, bbox.vx, bbox.vy, bbox.s])

    @classmethod
    def array2bbox(cls, data):
        bbox = BBox()
        bbox.x, bbox.y, bbox.z, bbox.o, bbox.l, bbox.w, bbox.h, bbox.vx, bbox.vy = data[:9]
        if len(data) == 10:
            bbox.s = data[9]
        return bbox
